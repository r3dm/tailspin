...
module.exports = function(CodeMirror) {
  ...
  function showTooltip(e, content) {
    ...
    function position(e) {
      if (!tt.parentNode) { return CodeMirror.off(doc, 'mousemove', position); }
      tt.style.top = Math.max(0, e.clientY - tt.offsetHeight - 5) + 'px';
      tt.style.left = (e.clientX + 5) + 'px';
    }
    CodeMirror.on(doc, 'mousemove', position);
    position(e);
    if (typeof tt.style.opacity !== 'undefined') { tt.style.opacity = 1; }
    return tt;
  }
  ...
  function hideTooltip(tt) {
    ...
    if (typeof tt.style.opacity === 'undefined') {
      rm(tt);
    }
    tt.style.opacity = 0;
  ...
  function makeMarker(labels, severity, multiple, tooltips) {
    var marker = doc.createElement('div'), inner = marker;
    marker.className = 'CodeMirror-lint-marker-' + severity;
    if (multiple) {
      inner = marker.appendChild(doc.createElement('div'));
  ...
  function annotationTooltip(ann) {
    ...
    tip.appendChild(doc.createTextNode(ann.message));
    return tip;
  }
  ...
  function updateLinting(cm, annotationsNotSorted) {
    ...
    for (var line = 0; line < annotations.length; ++line) {
      ...
      var tipLabel = state.hasGutter && doc.createDocumentFragment();
