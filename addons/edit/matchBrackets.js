...
module.exports = function(CodeMirror) {
  var IE8AndUp = (/MSIE \d/.test(navigator.userAgent)) &&
  ...
  function findMatchingBracket(cm, where, strict, config) {
    ...
    // this condition needs to be more readable
  ...
  function matchBrackets(cm, autoclear, config) {
    // Disable brace matching in long lines, since it'll cause slow updates
    ...
    for (var i = 0; i < ranges.length; i++) {
      if (ranges[i].empty()) {
        match = findMatchingBracket(cm, ranges[i].head, false, config);
      }
    ...
    if (marks.length) {
      ...
      if (IE8AndUp && cm.state.focused) { cm.display.input.focus(); }
